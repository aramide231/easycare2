...
export default function GenConsultProcedure() {
  const [procedure, setProcedure] = useState("");
  const [items, setItems] = useState<ProcedureItem[]>([]);
  const { history, save, remove } = useMedicalTable("GEN CONSULT — PROCEDURE");
                ...
                <tr key={index} className="border-b border-gray-100">
                  ...
                  <td className="px-3 py-2">
                    <input
                      ...
                      placeholder="Remarks"
                      className="h-8 w-full min-w-[120px] rounded border border-gray-300 px-2 text-sm"
                    />
                  </td>
                  <td className="px-3 py-2">
                    <button
