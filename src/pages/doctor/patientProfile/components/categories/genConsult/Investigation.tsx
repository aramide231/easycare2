...
const HISTORY_COLUMNS = [
  ...
  { key: "total", label: "AMOUNTS" },
  { key: "results", label: "RESULTS" },
];

export default function GenConsultInvestigation() {
  const [investigation, setInvestigation] = useState("");
  const [price, setPrice] = useState("");
  const [items, setItems] = useState<InvestigationItem[]>([]);

  const { history, save, remove } = useMedicalTable("GEN CONSULT — INVESTIGATION");
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end gap-3">
        <div className="min-w-[200px] flex-1">
          ...
          <input
            list="gen-consult-investigation-tariffs"
            value={investigation}
            onChange={(e) => pickTariff(e.target.value)}
            placeholder="Select Investigation (To Filter from uploaded tariffs...)"
            className={genConsultInputClass}
          />
          <datalist id="gen-consult-investigation-tariffs">
