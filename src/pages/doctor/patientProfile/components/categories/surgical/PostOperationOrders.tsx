...
export default function PostOperationOrders() {
  ...
  const { history, saveBatch, remove } = useMedicalTable(
    "SURGICAL — POST-OPERATION ORDERS",
  );
  ...
  const handleSave = () => {
    if (!items.length && !ordersNote.trim()) return;
    if (items.length) {
      saveBatch(
        items.map((item, index) => ({
          ...
          postOperativeOrders: index === 0 ? ordersNote : "",
        })),
      );
    } else {
      saveBatch([
        {
          ...
          drugForm: "—",
          dosage: "—",
          postOperativeOrders: ordersNote,
        },
      ]);
    }
    setItems([]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <label className={genConsultLabelClass}>Post Operative Orders</label>
        <textarea
          value={ordersNote}
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <div>
          <label className={genConsultLabelClass}>Treatment</label>
          <input
      ...
      <div className="text-center">
        <button
          type="button"
          onClick={handleSave}
          disabled={!items.length && !ordersNote.trim()}
          ...
          <tbody>
            {history.length === 0 ? (
              <tr>
                ...
                >
                  No post-operation orders recorded yet.
                </td>
              </tr>
            ) : (
