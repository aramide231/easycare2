...
import {
  buildMockPostNatalRecords,
  parsePostNatalDate,
} from "../data/mockPostNatalRecords";

const getCounsellingClass = (status: "DONE" | "NOT DONE"): string => {
  switch (status) {
    case "DONE":
...
const PostNatalLog = () => {
  ...
  const allRecords = useMemo(() => buildMockPostNatalRecords(), []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return allRecords.filter((record) => {
      const recordDate = parsePostNatalDate(record.followUp);
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          <ExportButton reportTitle="Post Natal Report" tableRef={tableRef} />
        </div>
      </div>
          ...
          <table ref={tableRef} className="w-full text-left text-sm">
            <thead className="border-b border-[#D4D4D4] text-xs uppercase text-gray-500">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">COUNSELLING</th>
              ...
              {paginatedItems.length > 0 ? (
                paginatedItems.map((record, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-md px-3 py-1 text-xs font-medium ${getCounsellingClass(record.counselling)}`}
                      >
                        {record.counselling}
                      </span>
                    </td>
                ...
                <tr>
                  ...
                  >
                    {search
                      ...
                      : "No post natal records for the selected date range."}
                  </td>
                </tr>
              )}
            </tbody>
