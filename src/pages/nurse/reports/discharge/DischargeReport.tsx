...
const DischargeReport = () => {
  const { dischargeReports } = usePatientManagement();
  ...
  const filteredRecords = useMemo(() => {
    ...
    return dischargeReports.filter((row) => {
      const recordDate = parseLogDateTime(row.dischargeDate, row.dischargeTime);
      const inRange =
        recordDate !== null &&
  ...
  }, [dischargeReports, search, dateRange]);
    ...
    <div className="w-full rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          <ExportButton reportTitle="Discharge Report" tableRef={tableRef} />
        </div>
      </div>
          ...
          <table ref={tableRef} className="min-w-full text-left text-sm">
            <thead className="border-b border-[#D4D4D4] text-xs uppercase text-gray-500">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">DATE OF ADM.</th>
                  ...
                  >
                    {search
                      ...
                      : dischargeReports.length === 0
                        ? "No discharge activities recorded yet. Take action on an admitted patient to see entries here."
                        : "No discharge records for the selected date range."}
                  </td>
                </tr>
              )}
            </tbody>
