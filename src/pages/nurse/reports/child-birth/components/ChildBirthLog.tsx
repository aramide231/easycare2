...
import {
  buildMockChildBirthRecords,
  parseDeliveryDate,
} from "../data/mockChildBirthRecords";

const ChildBirthLog = () => {
  const tableRef = useRef<HTMLTableElement>(null);
  const allRecords = useMemo(() => buildMockChildBirthRecords(), []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return allRecords.filter((record) => {
      const recordDate = parseDeliveryDate(record.deliveryDate);
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          <ExportButton reportTitle="Child Birth Report" tableRef={tableRef} />
        </div>
      </div>
          ...
          <table ref={tableRef} className="w-full text-left text-sm">
            <thead className="border-b border-[#D4D4D4] text-xs uppercase text-gray-500">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">BABY. G</th>
            ...
            <tbody>
              {paginatedItems.length > 0 ? (
                paginatedItems.map((record, index) => (
                  <tr key={record.id} className="border-b border-[#D4D4D4] bg-white">
                ...
                <tr>
                  ...
                  >
                    {search
                      ...
                      : "No child birth records for the selected date range."}
                  </td>
                </tr>
              )}
            </tbody>
