...
const PatientsLog: React.FC<PatientsLogProps> = ({
  ...
  const patients = controlledPatients ?? internalPatients;
  const setPatients = onPatientsChange ?? setInternalPatients;
  ...
  const filteredPatients = patients.filter((patient) => {
    ...
    const seenDate = parsePatientLastSeen(patient.lastSeen);
    if (!seenDate) return matchesSearch;

    const rangeStart = startOfDayDate(dateRange.startDate);
    const rangeEnd = startOfDayDate(dateRange.endDate);
    rangeEnd.setHours(23, 59, 59, 999);
    const normalizedSeen = startOfDayDate(seenDate);

    const inRange =
      normalizedSeen >= rangeStart && normalizedSeen <= rangeEnd;
    return matchesSearch && inRange;
  });

  const listToShow = searchTerm || dateRange ? filteredPatients : patients;
