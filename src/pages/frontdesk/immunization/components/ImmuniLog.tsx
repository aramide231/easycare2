...
import {
  buildMockImmunizationRecords,
  parseImmunizationDate,
} from "../data/mockImmunizationRecords";

const ImmunizationLog = () => {
  const tableRef = useRef<HTMLTableElement>(null);
  const allRecords = useMemo(() => buildMockImmunizationRecords(), []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return allRecords.filter((record) => {
      const recordDate = parseImmunizationDate(record.lastSeen);
      ...
      const matchesSearch =
        ...
        record.immunization.toLowerCase().includes(q);
              ...
              {paginatedItems.length > 0 ? (
                paginatedItems.map((record, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">{record.immunization}</td>
                ...
                <tr>
                  ...
                  >
                    {search
                      ...
                      : "No immunization records for the selected date range."}
                  </td>
                </tr>
              )}
            </tbody>
