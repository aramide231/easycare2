...
import TablePagination from "@/pages/diagnostics-and-radiologist/shared/components/TablePagination";
import {
  PATIENTS_LOG_ROWS,
  type DiagnosticsPatientsLogRow,
} from "@/pages/diagnostics-and-radiologist/data/mockDiagnostics";

const BASE = "/diagnostics-and-radiologist";
const SEARCH_PLACEHOLDER =
  "Search with Surname, Other Names, Unique ID, Enrollee Policy/Code Number or Telephone Number";
...
function PatientSearchModal({
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return PATIENTS_LOG_ROWS;
    return PATIENTS_LOG_ROWS.filter((patient) => {
      const fullName = `${patient.firstName} ${patient.lastName}`.toLowerCase();
      return (
        fullName.includes(q) ||
        patient.regName.toLowerCase().includes(q) ||
  ...
  const handleSelect = (patient: DiagnosticsPatientsLogRow) => {
    navigate(`${BASE}/patient-profile/${patient.id}`, {
      state: { patient },
    });
    onClose();
  };
...
export default function HeaderPatientSearch({
  className = "",
}: {
  className?: string;
}) {
