...
const SEED_RECEIPT_ITEMS: InvoicePaymentItem[] = [
  { id: "receipt-seed-1", label: "FEEDING", amount: 3000 },
  { id: "receipt-seed-2", label: "PROCEDURE", amount: 5000 },
  { id: "receipt-seed-3", label: "ULTRA SOUND", amount: 10000 },
  { id: "receipt-seed-4", label: "INVESTIGATION", amount: 5000 },
];
...
const ReceiptForm = ({
  ...
  const [items, setItems] = useState<InvoicePaymentItem[]>(SEED_RECEIPT_ITEMS);
  const [selectedLabel, setSelectedLabel] = useState("");
  const [priceInput, setPriceInput] = useState("");
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [amountPaidInput, setAmountPaidInput] = useState("20000");
  ...
  const amountPaid = parseNairaInput(amountPaidInput);
  ...
  const outstandingText =
    outstandingBalance > 0
      ...
      : formatNairaAmount(Math.abs(outstandingBalance));
      ...
      <div className="flex w-full flex-col items-center gap-8">
        <div className="flex w-full flex-col gap-6">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            <div className={fieldBoxClass}>{capturedAt}</div>
          </div>
            ...
            <div className="flex min-h-[45px] min-w-0 flex-1 flex-wrap items-center gap-2 overflow-hidden rounded-lg border-[0.5px] border-black bg-[#FAFAFA] p-1.5">
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  ...
                  >
                    {item.label === "ULTRA SOUND" ? "ULTRASOUND" : item.label}
                  </span>
                ))
              )}
            </div>
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            <div className={fieldBoxClass}>{formatNairaAmount(totalAmount)}</div>
          </div>
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-2 lg:flex-row lg:items-center lg:gap-2">
              ...
              <p className="shrink-0 text-[15px] font-semibold tracking-[-0.3px]">
                ...
                <span className="text-[#00C851]">{formatNairaAmount(0)}</span>
              </p>
            </div>
          </div>
