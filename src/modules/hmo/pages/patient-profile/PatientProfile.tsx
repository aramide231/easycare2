...
import FlagProfileReport from "./flag-profile/components/FlagProfileReport";
...
import ServiceFeeConfirmModal from "./components/ServiceFeeConfirmModal";
import PaymentHistoryForm from "./components/PaymentHistoryForm";
import AdmissionBillForm from "./components/AdmissionBillForm";
import AdmissionBillConfirmModal from "./components/AdmissionBillConfirmModal";
import InvoiceForm from "./components/InvoiceForm";
import ReceiptForm from "./components/ReceiptForm";
import type { ServiceFeeSection } from "./data/mockServiceFeeData";
import type { AdmissionBillSection } from "./data/mockAdmissionBillData";
...
const HmoPatientProfile = () => {
  ...
  const [serviceFeeSections, setServiceFeeSections] = useState<
    ServiceFeeSection[]
  >([]);
  const [serviceFeeConfirmOpen, setServiceFeeConfirmOpen] = useState(false);
  const [admissionBillSections, setAdmissionBillSections] = useState<
    AdmissionBillSection[]
  >([]);
  const [admissionBillConfirmOpen, setAdmissionBillConfirmOpen] =
    useState(false);

  const handleServiceFeeSectionsChange = useCallback(
    (sections: ServiceFeeSection[]) => {
      setServiceFeeSections(sections);
    },
    [],
  );

  const handleAdmissionBillSectionsChange = useCallback(
    (sections: AdmissionBillSection[]) => {
      setAdmissionBillSections(sections);
    },
    [],
  );
  ...
  const handleConfirm = () => {
    ...
    if (step === 2) {
      if (selectedCategory === "Service Fee") {
        setServiceFeeConfirmOpen(true);
        return;
      }
      if (selectedCategory === "Admission Bill") {
        setAdmissionBillConfirmOpen(true);
        return;
      }
  ...
  const handleServiceFeeConfirm = () => {
    setServiceFeeConfirmOpen(false);
    toast.success("Service fee bill confirmed.");
  };

  const handleAdmissionBillConfirm = () => {
    setAdmissionBillConfirmOpen(false);
    toast.success("Admission bill confirmed.");
  };
  ...
  const handlePreviousRecords = () => {
    navigate(`/hmo/previous-patient-records/${patient.patientId}`, {
      state: { patient },
    });
  };
  ...
  return (
    <div className="relative flex min-h-[calc(100dvh-7rem)] w-full max-w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="mt-2 flex w-full flex-1 flex-col">
          ...
          {step === 2 && (
            <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden">
              ...
              <h2 className="shrink-0 text-xs text-gray-400">Step 2</h2>
              <h3 className="mb-1.5 shrink-0 text-sm font-semibold text-gray-800">
                {selectedCategory === "Payment History"
                  ? "Payment History"
                  : selectedCategory === "Admission Bill"
                    ? "Admission Details"
              ...
              <div className="relative flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-lg border border-gray-200 bg-white">
                ...
                {selectedCategory === "Service Fee" && (
                  <div className="min-h-0 flex-1 overflow-y-auto p-4">
                    <ServiceFeeForm
                      onSectionsChange={handleServiceFeeSectionsChange}
                    />
                  </div>
                )}
                {selectedCategory === "Admission Bill" && (
                  <div className="min-h-0 flex-1 overflow-y-auto p-4">
                    <AdmissionBillForm
                      onSectionsChange={handleAdmissionBillSectionsChange}
                    />
                  </div>
                )}
                {selectedCategory === "Invoice" && (
                  <div className="min-h-0 flex-1 overflow-y-auto p-4">
                    <InvoiceForm
                      uniqueId="LDW-200"
                      surname={String(patient.lastName || "").toUpperCase()}
                      otherNames={`${String(patient.firstName || "").toUpperCase()} OLUWAPAC`}
      ...
      <FlagPatientPanel
        ...
        patientId={patient.patientId}
        variant="embedded"
        ReportComponent={FlagProfileReport}
      />
      <ServiceFeeConfirmModal
        open={serviceFeeConfirmOpen}
        patientName={`${patient.firstName} ${patient.lastName}`}
        patientId={patient.patientId}
        sections={serviceFeeSections}
        onClose={() => setServiceFeeConfirmOpen(false)}
        onConfirm={handleServiceFeeConfirm}
      />
      <AdmissionBillConfirmModal
        open={admissionBillConfirmOpen}
        patientName={`${patient.firstName} ${patient.lastName}`}
        patientId={patient.patientId}
        sections={admissionBillSections}
        onClose={() => setAdmissionBillConfirmOpen(false)}
        onConfirm={handleAdmissionBillConfirm}
      />
...
export default HmoPatientProfile;
