...
import {
  buildMockHmoAnteNatalLogRecords,
  parseHmoAnteNatalLogDate,
} from "../data/mockHmoAnteNatalLogRecords";

const AnteNatalLogsLog = () => {
  const tableRef = useRef<HTMLTableElement>(null);
  const allRecords = useMemo(() => buildMockHmoAnteNatalLogRecords(), []);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return allRecords.filter((record) => {
      const recordDate = parseHmoAnteNatalLogDate(record.bookDate);
      ...
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-3">
          <DateRangeFilter
            ...
            align="right"
          />
          <ExportButton reportTitle="Ante Natal Logs" tableRef={tableRef} />
        </div>
      </div>

      <div className="overflow-x-auto">
        <table ref={tableRef} className="w-full min-w-[960px] text-left text-sm">
...
export default AnteNatalLogsLog;
