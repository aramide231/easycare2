...
import {
  calcBatchAmount,
  DAMAGED_EXPIRED_SEED_ROWS,
  MEDICATION_OPTIONS,
  type DamagedExpiredRow,
...
export default function DamagedExpiredMedications() {
  const [selectedMedication, setSelectedMedication] = useState("");
  const [rows, setRows] = useState<DamagedExpiredRow[]>(
    () => DAMAGED_EXPIRED_SEED_ROWS
  );

  const updateRow = (
    id: number,
    field: keyof DamagedExpiredRow,
    value: string
  ) => {
    setRows((prev) =>
      prev.map((row) => {
  ...
  const handleConfirm = () => {
    toast.success("Damaged/expired medications confirmed.");
  };
          ...
          <tbody>
            {rows.length > 0 ? (
              ...
              <tr>
                ...
                >
                  No damaged or expired medications added.
                </td>
              </tr>
            )}
          </tbody>
